...
function mergeFragments(fragments, path) {
    ...
    fragments.forEach(fragment =>
        fragmentMap.getOrInsert(fragment.chunk.type, new Map())
    ...
    const mergedFragmentMap = new Map(fragmentMap.entries().map(([type, typedFragmentMap]) => {
        const innerMap = new Map(typedFragmentMap.entries().map(([value, fragments]) => {
            const currentPath = [...path, fragments[0].chunk];

            if (fragments.filter(fragment => fragment.handler).length > 1)
                throw new Error(`Multiple fragments provide handler function for /${chunksToString(currentPath)}`);
            ...
            if (fragments[0].chunk.type === "argument") {
                ...
                fragments.forEach(fragment => {
                    if (fragment.chunk.type !== "argument")
                        throw new Error("should be unreachable because chunk type should be the same as the first");

                    if (fragment.chunk.argumentTypeId !== expectedArgumentType)
                        throw new Error(`Expected all fragments of ${chunksToString(currentPath)} to have type ${expectedArgumentType}, but one fragment uses ${fragment.chunk.argumentTypeId}`);
                })
            }
            ...
            const mergedFragment = {
                ...
                children: mergeFragments(fragments.flatMap(fragment => Array.from(fragment.children)), currentPath)
            };
...
function fragment(chunks, handler, children = []) {
    ...
    return {
        ...
        children: chunks.length === 1 ? new Set(children) : new Set([fragment(chunks.slice(1), handler, children)])
    }
}
